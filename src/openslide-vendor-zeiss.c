...
static bool read_data_from_subblk(const char *filename, int64_t zisraw_offset,
  ...
  case COMP_ZSTD1:
    return czi_zstd_read(filename, data_pos, GINT64_FROM_LE(hdr->data_size),
                         sb->pixel_type, pixel_real_bits, dst, sb->compression,
                         NULL);
    break;
  ...
  case COMP_LZW:
    ...
    return false;
  default:
    g_warning("Unrecognized subblock format %d\n", sb->compression);
    return false;
  }
  return true;
}
...
static bool read_data_at_address(const char *filename, int64_t zisraw_offset,
                                 ...
                                 struct jxr_decoded *dst, GError **err) {
  struct jxr_decoded ch[MAX_CHANNEL];
  uint32_t *p;
  size_t gray8_len;
  size_t j = 0;
  ...
  if (ca->subblks[1] == NULL && ca->subblks[2] == NULL) {
    ...
    while (j < gray8_len) {
      *p++ = MERGE_GRAY_TO_ARGB(ch[0].data[j], ch[0].data[j], ch[0].data[j]);
      j++;
    }
    ...
    while (j < gray8_len) {
      *p++ = MERGE_GRAY_TO_ARGB(ch[0].data[j], ch[1].data[j], ch[2].data[j]);
      j++;
    }
  }
...
static bool read_tile(openslide_t *osr, cairo_t *cr,
  ...
  // file_pos is unique
  img = (unsigned char *)_openslide_cache_get(
      osr->cache, 0, ca->subblks[0]->file_pos, 0, &cache_entry);
  if (!img) {
    if (!read_data_at_address(data->filename, data->zisraw_offset, ca, &dst,
                              NULL)) {
    ...
    _openslide_cache_put(osr->cache, 0, ca->subblks[0]->file_pos, 0, dst.data,
                         dst.size, &cache_entry);
    img = (unsigned char *)dst.data;
  }

  stride = cairo_format_stride_for_width(CAIRO_FORMAT_RGB24, ca->tw);
  surface = cairo_image_surface_create_for_data(img, CAIRO_FORMAT_RGB24, ca->tw,
                                                ca->th, stride);
...
static void add_czi_address_to_grid(gpointer k G_GNUC_UNUSED, gpointer v,
                                    gpointer ops_data) {
  struct _openslide_grid *grid;
  struct czi_address *addr = v;
  struct zeiss_ops_data *data = ops_data;

  // make sure all scenes have same levels
  if (addr->downsample > data->common_downsample) {
    return;
  }
  ...
  _openslide_grid_range_add_tile(grid, (double)addr->x1 / addr->downsample,
...
static void init_range_grids(openslide_t *osr) {
  ...
  g_hash_table_foreach(data->addr_subblks, (GHFunc)add_czi_address_to_grid,
                       data);
...
static gboolean czi_addr_key_equal(gconstpointer a, gconstpointer b) {
...
static void destroy_addr_key(struct czi_address_key *k) { g_free(k); }

static void destroy_addr(struct czi_address *addr) { g_free(addr); }
...
static bool combine_subblks_from_multichannels(struct zeiss_ops_data *data,
  ...
  data->addr_subblks = g_hash_table_new_full(
      czi_addr_key_hash, czi_addr_key_equal, (GDestroyNotify)destroy_addr_key,
  ...
  for (guint i = 0; i < subblks->len; i++) {
    ...
    if (!addr) {
      addr = g_new0(struct czi_address, 1);
      ...
      key2 = g_new0(struct czi_address_key, 1);
      key2->x = key.x;
      key2->y = key.y;
...
static bool parse_xml_set_prop(openslide_t *osr, const char *xml,
  ...
  */
  ...
  g_autofree char *pixel_bits = _openslide_xml_xpath_get_string(
      ctx,
  ...
  g_autofree char *gamma = _openslide_xml_xpath_get_string(
      ctx,
  ...
  g_autofree char *camera = _openslide_xml_xpath_get_string(
      ctx, "/ImageDocument/Metadata/Information/Instrument/Detectors/Detector/"
